  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm" style={{ color: '#8a8a8a' }}>{user.username}</span>
        {user.role === 'admin' && (
          <button
            className="text-sm"
            style={{ background: '#242424', border: '1px solid #333', color: '#e5e5e5', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}
        ...
        <button
          className="text-sm"
          style={{ background: '#2a1f1f', border: '1px solid #4a2a2a', color: '#fca5a5', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}
          onClick={onLogout}
        >
          退出登录
        </button>
